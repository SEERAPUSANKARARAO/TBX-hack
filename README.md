...
- **No login, so a lock screen stands in for it**: a customer (`entity_id`) can genuinely own several accounts, often at different banks (the sample data models this deliberately — see `scripts/generate_sample_data.py`). On load, the UI shows a lock screen — pick a customer from the dropdown (`/api/entities`) and enter the demo password (`1234`, shared/public — this is explicitly **not real authentication**) to scope the whole session to that entity, or pick "All customers" to stay unscoped. Once set, scoping is enforced by both a prompt instruction and a post-generation check that the SQL actually filters by the selected `entity_id`. A "Switch" button in the left pane returns to the lock screen to change customers. **This is a usability gate, not a security boundary** (out of scope per the problem statement: no auth, no per-customer DB permissions).
...
Visit `http://localhost:8000`. On the lock screen, pick a customer (or "All customers") and enter password `1234` — several customers have accounts at 2+ banks on purpose, to exercise that case. Try the suggested question chips (just above the input bar), or ask your own — e.g. "How much did we send to Amazon Retail India this quarter?", "Show unreconciled transactions over 50000", "What's our balance at HDFC Bank?", "How much did we pay in IMPS charges this year?". Ask a follow-up like "and last month?" (or click one of the suggested follow-ups under each answer) to see multi-turn context reuse. Try the "unknown counterparty" chip to see the honest refusal path, or just say "hi" to see the greeting fast-path skip the SQL pipeline entirely.
